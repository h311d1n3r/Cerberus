...
class ELFHandler(AbstractELFHandler):
    ...
    def search_libs(self):
        with open(self.elf_path, 'rb') as elf_file:
            ...
            lib_matches = re.findall(b'/src/go/src/(.+?)\.(s|go)', elf_content)
            for lib_match in lib_matches:
                if lib_match[1] == b's':
                    continue
                lib_match = lib_match[0]
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                with open(goliath_dir + 'goliath', 'wb') as save_file:
                    ...
                    save_file.close()
                os.chmod(goliath_dir + 'goliath', 766)
    ...
    def download_and_build_libs(self, session_dir):
        ...
        init_output = subprocess.run(['goliath', 'mod', 'init', 'build_mod'], cwd=session_dir, capture_output=True)
        if b'creating new go.mod' not in init_output.stderr:
            info.fatal('Couldn\'t build module...')
            return False
        ...
        for lib_name in self.libs:
            ...
            download_output = subprocess.run(['goliath', 'get', lib_name+'@v'+lib_version], cwd=session_dir, capture_output=True)
        ...
        build_output = subprocess.run(['goliath', 'build', '-o', 'standalone/standalone.so', 'build_mod.go'], cwd=session_dir, capture_output=True)
