...
string ArgParser::format_help() {
    string res = "$(cyan)$(cyan:b)"+TOOL_TITLE+"$\n";
    res += " Version: $(white:b)"+VERSION+"$\n";
    res += " Author: $(white:b)"+AUTHOR+"$\n";
    res += "_____________________________________\n\n";
    res += "$(cyan:b)Syntax: $(red:b)cerberus binary [-param value] [--flag]$$\n\n";
    res += "$(cyan:b)Parameters:$\n";
    res += "   $(red:b)output (o)$ -> Specifies the path for the resulting ELF file. $(cyan:b)Default value: [input_binary]-patched$\n";
    ...
    res += "   $(red:b)debug (dbg)$ -> Enable debug level of logging.\n";
...
CONFIG* ArgParser::compute_args(int argc, char **argv) {
    ...
    config->output_path = this->parser.is_used("-output") ? this->parser.get<string>("output") : config->binary_path+"-patched";
    if(this->parser.is_used("-part_hash_len")) config->part_hash_len = this->parser.get<uint16_t>("part_hash_len");
    if(this->parser.is_used("-part_hash_trust")) config->part_hash_trust = this->parser.get<float>("part_hash_trust");
